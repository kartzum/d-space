...
void read() {
        ...
        prevV.resize(n, vector<Vertex> (m, Vertex(-1, -1)));
        cin >> st_col >> st_row;
        cin >> fin_col >> fin_row;
        --st_row; --st_col;
        --fin_row; --fin_col;
...
bool is_in_board(const Vertex &p) {
        return (p.row >= 0) && (p.row < n) && (p.column >= 0) && (p.column < m);
}
...
void solve() {
        ...
        while(!q.empty()) {
            ...
            for (int step = 0; step < num_of_steps; ++step) {
                ...
                if (is_in_board(neighbour) &&
                    ...
                    q.push(neighbour);
                    prevV[neighbour.row][neighbour.column] = v;
                }
            }
        }
        ...
        while(last_vertex != fake_vertex) {
            ...
            last_vertex = prevV[last_vertex.row][last_vertex.column];
        }
